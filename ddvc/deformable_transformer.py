...
from ddvc.ops.modules import MSDeformAttn
...
class DeformableTransformer(nn.Module):
    ...
    def prepare_text_encoder_inputs(self, srcs, masks, pos_embeds):
        # prepare input for encoder
        src_flatten = []
        mask_flatten = []
        lvl_pos_embed_flatten = []
    ...
    def forward_encoder_text(self, src_flatten, temporal_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten,
                        mask_flatten):
        # encoder
        if self.no_encoder:
            memory = src_flatten
        else:
            memory = self.text_encoder(src_flatten, temporal_shapes, level_start_index, valid_ratios, lvl_pos_embed_flatten,
                                  mask_flatten)
        return memory
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        # cross attention
        self.cross_attn_visual = MSDeformAttn(d_model, n_levels, n_heads, n_points)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)
        ...
        if args.text_crossAttn:
            self.text_cross = MSDeformAttn(d_model, n_levels, n_heads, n_points)
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_temporal_shapes, level_start_index,
                src_padding_mask=None, query_mask=None,text_feat=None, txt_temporal_shapes=None, txt_level_start_index=None, txt_mask_flatten=None,text_reference_points_input=None):
        ...
        if self.args.text_crossAttn_loc=="before":
            ...
            if text_feat is not None:
                tgt2 = self.text_cross(self.with_pos_embed(tgt, query_pos),
                                text_reference_points_input,
                                text_feat, txt_temporal_shapes, txt_level_start_index, txt_mask_flatten)
                tgt = tgt + self.dropout_t(tgt2)
                tgt = self.norm1(tgt)

        # cross attention
        tgt2 = self.cross_attn_visual(self.with_pos_embed(tgt, query_pos),
                               reference_points,
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_temporal_shapes, src_level_start_index, src_valid_ratios,
                query_pos=None, src_padding_mask=None, query_padding_mask=None, disable_iterative_refine=False,text_feat=None,
                txt_temporal_shapes=None, txt_level_start_index=None, txt_valid_ratios=None,txt_mask_flatten=None ):
        output = tgt
        ...
        for lid, layer in enumerate(self.layers):
            if reference_points.shape[-1] == 2:
                ...
                text_reference_points_input = reference_points[:, :, None] \
                                         * torch.stack([txt_valid_ratios, txt_valid_ratios], -1)[:, None]

            else:
                assert reference_points.shape[-1] == 1
                reference_points_input = reference_points[:, :, None] * src_valid_ratios[:, None, :, None]
                text_reference_points_input = reference_points[:, :, None] * txt_valid_ratios[:, None, :, None]

            output = layer(output, query_pos, reference_points_input, src, src_temporal_shapes, src_level_start_index,
                           src_padding_mask, query_padding_mask,text_feat,txt_temporal_shapes, txt_level_start_index,txt_mask_flatten,text_reference_points_input)

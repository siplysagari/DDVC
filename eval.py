...
import random
ddvc_dir = dirname(abspath(__file__))
...
from ddvc.ddvc_ret_encdec_clip import build
...
def main(opt):
    ...
    # vars(opt).update({"gpt_beam_size": 5})
    # print("beam_size",opt.gpt_beam_size)
...
if __name__ == '__main__':
    ...
    # opt.ret_text='token'
    # opt.down_proj='deep'
    # opt.eval_folder= 'yc2_clip_ddvc' 
    # # opt.eval_folder= 'anet_clip_ddvc' 
    # opt.exp_name = 'best' 
    # opt.eval_transformer_input_type ='queries' 
 
    opt.bank_path='data/bank'
    # if opt.eval_folder=='yc2_clip_ddvc':
    if "yc2" in opt.eval_folder:
        ...
        opt.target_domain='yc2'
    # elif opt.eval_folder=='anet_clip_ddvc':
    elif "anet" in  opt.eval_folder:
